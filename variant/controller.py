...
class _Presentation:
    ...
    corrections: dict[str, dict[str, CorrectionState]]
...
class VariantMainController:
    ...
    def _prepare(self, snapshot: BoardVariantSnapshot) -> _Presentation:
        """Calculate per-variant corrections and estimates once per source."""
        metadata: dict[tuple[str, str], CatalogMetadata] = {}
        corrections: dict[str, dict[str, CorrectionState]] = {}
        estimates: dict[str, BomEstimateResult] = {}
        ...
        for variant in snapshot.variants:
            ...
            matches = (
                resolve_shared_corrections(snapshot, rules, variant.name)
                if rules is not None
                else {}
            )
            variant_corrections: dict[str, CorrectionState] = {}
            for part in parts:
                if rules is None:
                    variant_corrections[part.component_id] = CorrectionState(
                        status="error"
                    )
                    continue
            ...
            corrections[variant.name] = variant_corrections
    ...
    def action_correction(self, target: Any = None) -> None:
        ...
        variant_name = self.session.output_variant
        part = snapshot.get(target.component_id, variant_name)
        ...
        try:
            with CorrectionManagerDialog(self.dialog, pattern) as manager:
                ...
                if stored.corrections is not None:
                    match = resolve_shared_corrections(
                        snapshot, stored.corrections, variant_name
                    )[part.component_id]
                    if match is not None:
                        ...
                        if record is not None:
                            manager.populate_corrections_list(
                                selected=(record.kind, record.rowid)
                            )
                manager.ShowModal()
        finally:
            self.refresh()
