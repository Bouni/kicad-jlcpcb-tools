...
_SHARED_COLUMNS = (
    ...
    ColumnSpec(
        ...
        "Corr.",
        None,
        tooltip="Shared physical rotation and XY placement correction, resolved from base fields",
    ),
)
_VARIANT_COLUMNS = (
...
class CorrectionState:
    """A validated shared correction with its prepared physical placement angle."""
...
class MatrixModel:
    ...
    def _correction_for(self, component_id: str) -> CorrectionState:
        """Read prepared shared results; omitted components are not yet resolved."""
    ...
    def cell_details(self, row: int, column: int) -> str:
        ...
        if spec.key == "lcsc" and value:
            ...
            details = [
                details[0],
                f"Physical correction shared by all variants; resolved from {self.variant_label('')} (base).",
            ]
