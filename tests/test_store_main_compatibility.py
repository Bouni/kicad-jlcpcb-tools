...
def test_ordinary_csv_import_keeps_backup_behavior_without_new_marker(
    tmp_path: Path, store_module: ModuleType
) -> None:
    """Import through reopening, preserve assigned values, and rename the source."""
    _store(store_module, tmp_path)
    source = tmp_path / "jlcpcb" / "part_assignments.csv"
    source.write_text("R1,C123,1,1\n", encoding="utf-8")

    imported = _store(store_module, tmp_path)

    assert imported.get_part("R1")["lcsc"] == "C123"
    assert imported.get_part("R1")["exclude_from_bom"] == 1
    assert imported.get_part("R1")["exclude_from_pos"] == 1
    assert not source.exists()
    assert (
        source.with_suffix(".csv.backup").read_text(encoding="utf-8") == "R1,C123,1,1\n"
    )
    assert _metadata(imported) == []
