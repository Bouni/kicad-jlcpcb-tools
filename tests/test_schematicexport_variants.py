...
"""
        if version.startswith("6."):
            text = text.replace('"R1" (at', '"R1" (id 0) (at').replace(
                '"C100" (at', '"C100" (id 4) (at'
            )
        return text
    return """(kicad_sch
  (symbol
...
@pytest.mark.parametrize("source", ["6.0.11", "7.0.11", "10.0.6"], indirect=True)
@pytest.mark.parametrize(
...
def test_base_bom_observes_all_instances_and_preserves_variant_fields(
    ...
    overrides = ""
    if source.version.startswith("6."):
        (source.directory / "board.kicad_sch").write_text(
            "(kicad_sch\n  (symbol_instances\n"
            '    (path "/first/symbol-uuid" (reference "R1") (unit 1))\n'
            '    (path "/second/symbol-uuid" (reference "R2") (unit 1))\n  )\n)\n',
            encoding="utf-8",
        )
    else:
        overrides = """    (instances
