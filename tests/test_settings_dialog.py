"""Regression tests for the settings dialog's labels and state.
...
@pytest.mark.parametrize("priority", [True, False, None])
def test_lcsc_priority_setting_is_inert_and_has_no_control(
    priority: Optional[bool],
) -> None:
    """Old precedence settings are left untouched and never offered in the UI."""
    ...
    assert not hasattr(dialog, "lcsc_priority_image")
    assert _wx.posted_events == []
    if priority is None:
        assert "lcsc_priority" not in settings_dict["general"]
    else:
        assert settings_dict["general"]["lcsc_priority"] is priority


def test_default_settings_have_no_lcsc_precedence_option() -> None:
    """Fresh configuration does not advertise the retired project database source."""
    defaults = json.loads(
        (Path(__file__).parent.parent / "default_settings.json").read_text()
    )
    assert "lcsc_priority" not in defaults["general"]
