...
def test_unavailable_context_rejects_before_mutation(
    ...
    if failure == "board":
        editor.board = SimpleNamespace(
            m_Uuid=SimpleNamespace(AsString=lambda: "reloaded-same-filename"),
            fields={"R1": "C999"},
        )
    elif failure == "editor":
        editor.deleted = True
...
def test_old_window_cannot_retarget_after_new_board_launch(harness: Any) -> None:
    ...
    harness.editor.board = SimpleNamespace(
        m_Uuid=SimpleNamespace(AsString=lambda: "new-board"), fields={"R1": "C999"}
    )
...
def test_public_uuid_identifies_new_wrapper_of_same_native_board(harness: Any) -> None:
...
def test_context_is_rechecked_inside_native_callback(harness: Any) -> None:
    ...
    def switch_then_dispatch(event: Any) -> bool:
        harness.editor.board = SimpleNamespace(
            m_Uuid=SimpleNamespace(AsString=lambda: "replacement"), fields={}
        )
