...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("add_correction", "corrections", "CorrectionManagerDialog", "LCSC", ""),
    ],
)
...
def test_ordinary_callers_destroy_after_modal_exit_and_reopen(
    ...
    def create(parent: Any, *args: object, **kwargs: object) -> ModalDialog:
        assert parent is window
        assert args == (() if expected_filter is None else (expected_filter,))
        assert kwargs == ({"lcsc_part": "C123"} if filter_id == "LCSC" else {})
        child = ModalDialog()
